...
void main(void){ 
    ...
    //  normalMatrix = transpose(normalMatrix);
     ...
     //sampler2D
     albedoTexColor = texture2D(albedoTex, uv_0 );// vec4(0.2,0.2,0.2,1.0);//
     normalTexColor = vec3(0.0,1.0,0.0);//unpackNormal(texture2D(normalTex, uv_0 )) ; 
     opacityTexColor = vec4(0.0,0.0,0.0,0.0);//(opacityTex, uv_0 ) ; 
     glossTexColor = vec4(0.0,0.0,0.0,0.0);//texture2D(glossTex, uv_0 );
     specularTexColor = vec4(0.0,0.0,0.0,0.0);//texture2D(specularTex, uv_0 );
     // Perturbed normals
