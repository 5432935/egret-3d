module egret3d {
    // 按键播放 1. 2. 3. 4
    export class Class_SkeletonAnimationOutLine extends Class_View3D {
        ...
        private onEAMLoadComplete(e: LoaderEvent3D): void {
            ...
            if (mesh.animation.animStateNames.length == e.loader["eamLength"]) {
                ...
                for (var x: number = 0; x < 10; x++) {
                    ...
                    cloneMesh.material.addDiffuseChilderPass(pass);
                    //调整绘制外线框颜色
                    pass.outLineColor = 0xff00ff00;
                    //调整绘制外线框size
                    pass.outLineSize = 0.2;
                    var flag = Math.random() * 1.0 > 0.5 ? true : false ;
                    pass.enable = flag ;
                }
