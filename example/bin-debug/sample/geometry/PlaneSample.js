/**
 * 平面模型
 *
 */
var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
var PlaneSample = (function () {
    function PlaneSample() {
        StageMgr.Instance().init();
        this.init();
    }
    PlaneSample.prototype.init = function () {
        var geom = new egret3d.PlaneGeometry(50, 100, 10);
        var mat = new egret3d.TextureMaterial();
        var cube = new egret3d.Mesh(geom, mat);
        StageMgr.Instance().view3d.addChild3D(cube);
        this.InitCameraCtl();
        StageMgr.Instance().stage3d.addEventListener(egret3d.Event3D.ENTER_FRAME, this.update, this);
    };
    PlaneSample.prototype.InitCameraCtl = function () {
        this.cameraCtl = new egret3d.LookAtController(StageMgr.Instance().view3d.camera3D, new egret3d.Object3D());
        this.cameraCtl.distance = 300;
        this.cameraCtl.rotationX = 40;
    };
    PlaneSample.prototype.update = function (e) {
        this.cameraCtl.update();
    };
    return PlaneSample;
}());
__reflect(PlaneSample.prototype, "PlaneSample");
//# sourceMappingURL=PlaneSample.js.map