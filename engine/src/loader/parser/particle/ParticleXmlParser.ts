module egret3d {
    ...
    export class ParticleXmlParser {
        ...
        */
         ...
         */
        public parse(xml:any, data:ParticleData): void {
            this._particleData = data;

            this.version = this.getNode(xml, "version").textContent;
            //property
            var propertyNode: Node = this.getNode(xml,"property");
            this.parseProperty(propertyNode);
            //emission
            var emissionNode: Node = this.getNode(xml, "emission");
            this.parseEmission(emissionNode);
            //life
            var life: Node = this.getNode(xml, "life");
            this.parseLife(life);
            //shape
            var shape: Node = this.getNode(xml, "shape");
            this.parseShape(shape);
            //rotationBirth
            var rotationBirth: Node = this.getNode(xml, "rotationBirth");
            this.parseRotationBirth(rotationBirth);
            //scaleBirth
            var scaleBirth: Node = this.getNode(xml, "scaleBirth");
            this.parseScaleBirth(scaleBirth);
            //geometry
            var geometry: Node = this.getNode(xml, "geometry");
            this.parseGeometry(geometry);
            //moveSpeed
            var moveSpeed: Node = this.getNode(xml, "moveSpeed");
            this.parseMoveSpeed(moveSpeed);
            //followTarget
            var followTarget: Node = this.getNode(xml, "followTarget");
            this.parseFollowTarget(followTarget);
            //parseBezierNode
            var scaleBezier: Node = this.getNode(xml, "scaleBezier");
            this.parseScaleBeizer(scaleBezier);
            //rotationSpeed
            var rotationSpeed: Node = this.getNode(xml, "rotationSpeed");
            this.parseRotationSpeed(rotationSpeed);
            //colorOffset
            var colorOffset: Node = this.getNode(xml, "colorOffset");
            this.parseColorOffset(colorOffset);
            //material
            var material: Node = this.getNode(xml, "mat");
            //textureSheet
            var textureSheet: Node = this.getNode(xml, "textureSheet");
        ...
        private parseShape(node: Node): void {
            ...
            var emitFromShell:Node = this.getNode(node, "emitFromShell");
        ...
        */
         ...
         * 解析一个vector3D数据
         */
        private parseVector3D(node: Node, vector: Vector3): Vector3 {
            if (vector == null)
                vector = new Vector3();
