module egret3d {
    ...
    export class OctahedronSphereGeometry extends Geometry {
        ...
        private vector3_Down: Vector3 = new Vector3(0, -1, 0);
        private vector3_Forward: Vector3 = new Vector3(0, 0, 1);
        private vector3_Up: Vector3 = new Vector3(0, 1, 0);
        ...
        */
        ...
        private CreateOctahedron(vertices: Vector3[], triangles: number[], resolution: number, isHemisphere: boolean) {
            ...
            if (isHemisphere == false) {
                ...
                for (var i = 1; i <= resolution; i++) {
                    ...
                    var _from: Vector3 = new Vector3();
                    var _to: Vector3 = new Vector3();
        ...
        private CreateVertexLine(_from: Vector3, _to: Vector3, steps: number, v: number, vertices: Vector3[]): number {
            for (var i = 1; i <= steps; i++) {
        ...
        private Normalize(vertices: Vector3[], normals: Vector3[], front: boolean) {
            for (var i = 0; i < vertices.length; i++) {
                var vec = vertices[i];
                vec.normalize();
                normals[i] = vec;
        ...
        private CreateUV(vertices: Vector3[], uv: UV[]) {
            var previousX = 1.0;
            for (var i = 0; i < vertices.length; i++) {
                var v = vertices[i];
                if (v.x == previousX) {
        ...
        private CreateTangents(vertices: Vector3[], tangents: Quaternion[]) {
            for (var i = 0; i < vertices.length; i++) {
                var v = vertices[i];
                v.y = 0;
                v.normalize();
