    ...
    */
    export class Object3D extends EventDispatcher {
        ...
        */
        ...
        protected static mat0: Matrix4 = new Matrix4();
        ...
        * @platform Web,Native
        */
        public set modelMatrix(matrix: Matrix4)  {
            var tas: Vector3[] = matrix.decompose(Orientation3D.QUATERNION);
        ...
        * @platform Web,Native
        */
        public set localMatrix(matrix: Matrix4) {
            var tas: Vector3[] = matrix.decompose(Orientation3D.QUATERNION);
            this.position = tas[0];
        ...
        */
        protected updateModelMatrix() {
            ...
            if (this.parent != null) {
                ...
                var parentScale: Vector3 = this.parent.globalScale;
        ...
        * @platform Web,Native
        */
        public set globalPosition(pos: Vector3) {
            if (this.parent) {
        ...
        * @platform Web,Native
        */
        public set globalScale(sca: Vector3) {
            if (this.parent) {
        ...
        */
        public lookAt(pos: Vector3, target: Vector3, up: Vector3 = Vector3.Y_AXIS) {
            ...
            var prs: Vector3[] = MathUtil.CALCULATION_MATRIX.decompose(Orientation3D.QUATERNION);
        ...
        * @platform Web,Native
        */
        public lookAtLocal(pos: Vector3, target: Vector3, up: Vector3 = Vector3.Y_AXIS) {
            this.position = pos;
            MathUtil.CALCULATION_MATRIX.lookAt(pos, target, up);
            MathUtil.CALCULATION_MATRIX.invert();
            var prs: Vector3[] = MathUtil.CALCULATION_MATRIX.decompose(Orientation3D.QUATERNION);
        ...
        */
        public lookAtTarget(target: Object3D) {
            MathUtil.CALCULATION_MATRIX.lookAt(this.globalPosition, target.globalPosition, Vector3.Y_AXIS);
            MathUtil.CALCULATION_MATRIX.invert();
            var prs: Vector3[] = MathUtil.CALCULATION_MATRIX.decompose(Orientation3D.QUATERNION);
            MathUtil.CALCULATION_QUATERNION.x = prs[1].x;
            MathUtil.CALCULATION_QUATERNION.y = prs[1].y;
        ...
        */
        public lookAtTargetLocal(target: Object3D) {
            MathUtil.CALCULATION_MATRIX.lookAt(this.position, target.position, Vector3.Y_AXIS);
            MathUtil.CALCULATION_MATRIX.invert();
            var prs: Vector3[] = MathUtil.CALCULATION_MATRIX.decompose(Orientation3D.QUATERNION);
            MathUtil.CALCULATION_QUATERNION.x = prs[1].x;
            MathUtil.CALCULATION_QUATERNION.y = prs[1].y;
        ...
        public update(time: number, delay: number, camera: Camera3D) {
            ...
            if (this._displayList) {
                for (var i: number = 0; i < this._displayList.length; i++) {
                    camera.object3DToScreenRay(this.globalPosition, Vector3.HELP_0);
                    this._displayList[i].x = Vector3.HELP_0.x;
                    this._displayList[i].y = Vector3.HELP_0.y;
                }
            }
        }
