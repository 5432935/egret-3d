module egret3d {
    ...
    */
    export class ConeValueShape extends ValueShape {
        ...
        private static randomPosTop: Vector3 = new Vector3();
        //在圆锥体的顶部随机一个位置
        private randomPosAtTop(): void{
            var pos: Vector3 = ConeValueShape.randomPosTop;
            var values: Vector3[] = [];
            var tempAngle: number;
            var targetRadius: number;
            tempAngle = Math.random() * Math.PI * 2;
        ...
        * @private 获取一个随机方向
        */
        public randomDirectionToTop(result:Vector3): void {
            this.randomPosAtTop();
    ...
    */
    class LineValueShape extends ValueShape {
        ...
        public points: Vector3[] = [new Vector3(), new Vector3(100, 0, 0), new Vector3(100, 200, 0)];
        ...
        public calculate(num: number): any {
            ...
            for (var i: number = 1; i < this.points.length; i++) {
                numLen += Vector3.distance(this.points[i - 1], this.points[i]);
            }
            segment = numLen / num;
            var ntmp: Vector3 = new Vector3();
            ...
            for (var i: number = 1; i < this.points.length; i++) {
                ...
                sourceD = Vector3.distance(this.points[i - 1], this.points[i]);
                nD = Vector3.distance(ntmp, this.points[i]);
    ...
    */
    export class BallValueShape extends ValueShape {
        ...
        private getPoints1(num: number, r: number): Vector3[] {
            var values: Vector3[] = [];
            var x: number;
            var y: number;
            var z: number;
            var pos: Vector3;
            var radio: Vector3 = new Vector3(0, 0, 0);
            for (var i: number = 0; i < num; i++) {
                x = Math.random() * 2 * r - r;
                y = Math.random() * 2 * r - r;
                z = Math.random() * 2 * r - r;
                ...
                if (Vector3.distance(radio, pos) > r) {
                    i--;
                } else {
                    values.push(pos);
                }
    ...
    */
    export class HemiBallValueShape extends ValueShape {
        ...
        private getPoints(num: number, r: number): Vector3[] {
            var values: Vector3[] = [];
            var x: number;
            var y: number;
            var z: number;
    ...
    */
    class CircleValueShape extends ValueShape {
        ...
        private createRandomPoint1(num: number, r: number): Vector3[] {
            var values: Vector3[] = [];
            var d: number = r * 2;
            for (var i: number = 0; i < num; i++) {
                ...
                if ((x * x + z * z) > (r * r)) {
                    ...
                    values.push(new Vector3(x, 0, z));
                }
            }
            return values;
        }

        private createRandomPoint2(num: number, r: number): Vector3[] {
            var values: Vector3[] = [];
            var pos: Vector3;
            var tempR: number;
            var theta: number;
    ...
    export class Mesh3DValueShape extends ValueShape {
        private static vct1: Vector3 = new Vector3();
        private static vct2: Vector3 = new Vector3();
        private static vct3: Vector3 = new Vector3();

        public valueType: ValueType = ValueType.vec3;
        public normalList: Vector3[] = [];
        ...
        */
        ...
        private edgePosition(values:Vector3[], num:number): void {
            var val: Vector3;
            var normal: Vector3;
        ...
        private trianglePosition(values: Vector3[], num: number): void {
            var val: Vector3;
            var normal: Vector3;

            var triangleCount: number = this.geometry.faceCount;
            var vc1: Vector3 = new Vector3();
            var vc2: Vector3 = new Vector3();
            var random: number;
        ...
        private vertexPosition(values: Vector3[], num: number): void {
            var val: Vector3;
            var normal: Vector3;
        ...
        private static crsVector1: Vector3 = new Vector3();
        private static crsVector2: Vector3 = new Vector3();
        private normal: Vector3;
        private calcNormal(pt0: Vector3, pt1: Vector3, pt2: Vector3): Vector3 {
    ...
    */
    class BezierCurveValueShape extends ValueShape {
        ...
        public calculate(num: number, ...parameters): any {
            ...
            //var tmpPar = parameters[0];
            var tmpPar: Vector3[] = [];
            tmpPar.push(new Vector3(0, 0, 0));
            tmpPar.push(new Vector3(-200, 1000, 700));
            tmpPar.push(new Vector3(200, -50, 300));
            tmpPar.push(new Vector3(-300, -220, 500));

            var p0: Vector3 = tmpPar[0];
            var p1: Vector3 = tmpPar[1];
            var p2: Vector3 = tmpPar[2];
            var p3: Vector3 = tmpPar[3];
            var t: number;
            var yt: number;
            var x: number;
            var y: number;
   ...
   */
    export class ValueShapeExternal extends ValueShape {
        public valueType: ValueType = ValueType.vec3;
        public positionList: Vector3[];

        public calculate(num: number, ...parameters): any {
            var values:Vector3[] = this.positionList.slice();
            return values;
        }
    }
