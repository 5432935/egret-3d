module egret3d {
    ...
    */
    export class BoundBox extends Bound {
        ...
        * @platform Web,Native
        */
        constructor(owner: Object3D = null, min: Vector3 = null, max: Vector3 = null) {
            super(owner);
            if (!min) {
                min = new Vector3();
            }
        ...
        * @platform Web,Native
        */
        public fillBox(min: Vector3, max: Vector3) {
            this.min.copyFrom(min);
            this.max.copyFrom(max);
            this.calculateBox();
        }
        ...
        */
        public calculateBox() {
            ...
            var sub: Vector3 = this.max.subtract(this.min, MathUtil.CALCULATION_VECTOR3D_0);
            ...
            var c: Vector3 = this.max.subtract(this.min, MathUtil.CALCULATION_VECTOR3D_1);
            c.scaleBy(0.5);

            this.radius = c.length;
            this.center.copyFrom(this.min);
            var tmp: Vector3 = this.center.add(c, MathUtil.CALCULATION_VECTOR3D_2);
        ...
        */
        public inBound(frustum: Frustum): boolean {
            ...
            let radius = this.transform.getMaxScaleOnAxis() * this.radius;
            return frustum.inSphere(MathUtil.CALCULATION_VECTOR3D, radius);
        }

        protected updateAABB() {
            this.min.copyFrom(new Vector3(MathUtil.MAX_VALUE, MathUtil.MAX_VALUE, MathUtil.MAX_VALUE));
            this.max.copyFrom(new Vector3(-MathUtil.MAX_VALUE, -MathUtil.MAX_VALUE, -MathUtil.MAX_VALUE));
