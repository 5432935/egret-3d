    ...
    */
    export class PickSystem {
        ...
        */
        ...
        public getObjectId(x: number, y: number, cavans:Stage3D, view: View3D): number {
    ...
    */
    export class EyePick extends EventDispatcher {
        ...
        public update(view: View3D, time: number, delay: number) {
            ...
            if (this._selection)
                minDis = Vector3.distance(view.camera3D.position, this._selection.position);;
            var dis: number = 0;
            ...
            for (var i: number = 0; i < pickList.length; i++) {
                ...
                if (this._sceneRay.IntersectMesh(bound.vexData, bound.indexData, bound.vexLength, bound.indexData.length / 3, 0, pickList[i].modelMatrix, pickList[i].pickResult)) {
                    //bound.visible = true;
                    this._select = true;
                    dis = Vector3.distance(view.camera3D.position, pickList[i].position);
                    if (dis < minDis) {
                        this._count = 0;
                        minDis = dis;
                        if (this._selection){
